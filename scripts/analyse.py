...
def main():
    ...
    for item in ["walltime", "n_iterations"]:
        ...
        for tol in [1e-2, 1e-3]:
            for solver in ["sbgrd", "bundle", "mosek", "cosmo", "cosmon", "sdpnal"]:
            # for solver in ["sbgrd", "mosek", "bundle"]:
                if solver == "cosmon":
                    continue
                print(f" {sep} ", end="")
        ...
        for input_file_path in input_file_paths1:
            ...
            for tol in [1e-2, 1e-3]:
                values = []
                for solver in ["subgradient_projection", "bundle", "mosek", "cosmo", "cosmon", "sdpnal"]:
                # for solver in ["subgradient_projection", "mosek", "bundle"]:
                    if solver == "cosmon":
                        continue
    ...
    for problem_name in ["gpp", "mcp"]:
        ...
        for density in [5, 10]:
            for tol_i, tol in enumerate([1e-2, 1e-3]):
                for solver_i, solver in enumerate(["sbgrd", "bundle", "mosek"]):
                    print(f" {sep} ", end="")
        ...
        for size in [1000, 2000, 3000, 4000, 5000]:
            for random_seed in [1, 2, 3, 4]:
                ...
                for density in [5, 10]:
                    ...
                    for tol in [1e-2, 1e-3]:
                        values = []
                        for solver in ["subgradient_projection", "bundle", "mosek"]:
...
def load_walltime(input_file_path, tol, solver, item="walltime"):
    ...
    if solver == "bundle":
        try:
            problem_name = os.path.splitext(os.path.basename(input_file_path))[0]
            output_file_path = f"outputs/v3/ConicBundle/tol_{tol}_{problem_name}.txt"
            with open(output_file_path, "r") as f:
                res = yaml.safe_load(f)
            return res[item]
        except FileNotFoundError:
